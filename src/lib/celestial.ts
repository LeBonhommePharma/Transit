/** Local sun/moon azimuth and altitude. NOAA/Meeus-style; no network, no packages. */
...
export type LightBody = Horizontal & {
  source: "sun" | "moon";
};
...
function asDate(time: unknown): Date | null {
...
function asLatLon(lat: unknown, lon: unknown): { lat: number; lon: number } | null {
...
function eclipticToRaDec(lonDeg: number, latDeg: number, T: number): { ra: number; dec: number } {
...
function horizontalFromRaDec(lat: number, lon: number, raDeg: number, decDeg: number, jd: number): Horizontal {
...
function sunEcliptic(T: number): { lon: number; lat: number } {
...
function moonEcliptic(T: number): { lon: number; lat: number } {
...
function bodyHorizontal(
  ...
  lon: number,
  date: Date,
  ecliptic: (T: number) => { lon: number; lat: number },
): Horizontal {
...
/** Sun azimuth (0=N, 90=E) and altitude in degrees. Junk → null. */
export function sunAzAlt(lat: unknown, lon: unknown, time: unknown): Horizontal | null {
...
/** Moon azimuth (0=N, 90=E) and altitude in degrees. Junk → null. */
export function moonAzAlt(lat: unknown, lon: unknown, time: unknown): Horizontal | null {
  const ll = asLatLon(lat, lon);
  const date = asDate(time);
  if (!ll || !date) return null;
...
/**
 * Geometric moon rise and the following set around `around`.
 * Rise/set are geometric altitude zero-crossings of this same model.
 */
export function moonRiseSet(lat: unknown, lon: unknown, around: unknown): MoonRiseSet | null {
  const ll = asLatLon(lat, lon);
...
function interpolateZero(t0: number, a0: number, t1: number, a1: number): Date {
  const span = a1 - a0;
  const f = Math.abs(span) < 1e-9 ? 0 : (0 - a0) / span;
  return new Date(t0 + clamp(f, 0, 1) * (t1 - t0));
}

/** Daytime: the sun. After the sun is below the horizon: the moon, if it is up. Else null. */
export function observerLight(lat: unknown, lon: unknown, time: unknown): LightBody | null {
